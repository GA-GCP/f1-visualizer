...
P=f1v-example-project

# The bucket this unit's own state is stored in (SEC-7).
terragrunt import 'google_storage_bucket.tfstate' "$P/f1v-example-project-tfstate"
...
```bash
gsutil rm -r gs://f1v-example-project-tfstate/dev/artifact-registry
gsutil rm -r gs://f1v-example-project-tfstate/uat/artifact-registry
```
...
gcloud certificate-manager certificates describe f1v-frontend-dev-cert-managed \
  --location=global --project=f1v-example-project --format='value(managed.state)'
