 ...
 *
 * <p>C4: {@code "f1_dataset"} was a private constant in ten classes across two
 * services, while {@code spring.cloud.gcp.bigquery.dataset-name} sat in every
 * YAML file and was read by nothing. C6: this is validated, so a missing or blank
 * value fails at startup rather than at the first query.</p>
 */
@Validated
...
public record BigQueryProperties(
        ...
        @Positive long maximumBytesBilled) {
    ...
    public static BigQueryProperties defaults() {
        return new BigQueryProperties("f1_dataset", DEFAULT_JOB_TIMEOUT_MS, DEFAULT_MAXIMUM_BYTES_BILLED);
    }
