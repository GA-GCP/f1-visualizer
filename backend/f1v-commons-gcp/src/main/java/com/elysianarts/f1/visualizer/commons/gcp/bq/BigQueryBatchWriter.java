 ...
 *
 * <p><b>Why not {@code insertAll} (R4).</b> The four session loaders called it
 * with no {@code insertId} and never cleared existing rows, so re-running a load
 * inserted everything a second time and the lap-time chart doubled. Worse, rows
 * written that way sit in the streaming buffer for up to about 90 minutes and
 * cannot be removed by DML — which is why {@code ReferenceDataLoader}'s
 * delete-then-insert could leave stale rows behind when run twice in a row.</p>
 *
 * <p>A batch load job has neither problem: the rows are immediately visible to
 * DML, so a load can safely delete what a previous run wrote and replace it. It
 * also costs nothing, where streaming inserts are billed per byte.</p>
 */
@Slf4j
@Component
public class BigQueryBatchWriter {
    ...
    public long append(String dataset, String table, List<Map<String, Object>> rows) {
        ...
        WriteChannelConfiguration configuration = WriteChannelConfiguration
                .newBuilder(TableId.of(dataset, table))
        ...
        try (TableDataWriteChannel writer = bigQuery.writer(configuration)) {
            for (Map<String, Object> row : rows) {
                ...
                byte[] line = (mapper.writeValueAsString(row) + "\n").getBytes(StandardCharsets.UTF_8);
            ...
            if (job.getStatus().getError() != null) {
                throw new IllegalStateException("BigQuery load job failed: " + job.getStatus().getError());
            }
