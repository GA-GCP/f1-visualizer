...
class IngestionControllerTest {
    ...
    @MockitoBean private HistoricalDataLoader historicalDataLoader;

    @MockitoBean private ReferenceDataLoader referenceDataLoader;

    @MockitoBean private ReplayCommandStream commandStream;

    @MockitoBean private ReplayStateStore stateStore;

    @MockitoBean private LapDataLoader lapDataLoader;

    @MockitoBean private ResultDataLoader resultDataLoader;

    @MockitoBean private LocationDataLoader locationDataLoader;

    @MockitoBean private DriverStatsPrecomputer driverStatsPrecomputer;

    @MockitoBean private IngestionJobService jobService;

    @MockitoBean private JwtDecoder jwtDecoder;

    @MockitoBean private OpenF1CredentialsConfig.OpenF1Credentials openF1Credentials;
    ...
    void seekSimulation_CallsEngineWithPercentage() throws Exception {
        mockMvc.perform(
                        post("/api/v1/ingestion/playback/seek")
                                .param("percentage", "75")
                                .with(viewer()))
                .andExpect(status().isOk())
    ...
    /**
     * S1: this is the finding. Any signed-in user could switch the global ingestion mode, and
     * delete and rewrite the BigQuery reference tables.
     */
    @Test
    ...
    void loadReference_Returns403_WhenCallerLacksIngestAdmin() throws Exception {
        mockMvc.perform(
                        post("/api/v1/ingestion/load-reference")
                                .param("year", "2023")
                                .with(viewer()))
                .andExpect(status().isForbidden());
    ...
    void loadHistorical_Returns403_WhenCallerLacksIngestAdmin() throws Exception {
        mockMvc.perform(
                        post("/api/v1/ingestion/load-historical")
                                .param("sessionKey", "9165")
                                .with(viewer()))
                .andExpect(status().isForbidden());
    ...
    void loadHistorical_Returns202WithAJob() throws Exception {
        ...
        when(jobService.submit(eq(IngestionJob.Type.HISTORICAL), eq("9165"), any()))
                .thenReturn(job);
    ...
    void getJob_ReturnsTheJob_WhenItExists() throws Exception {
        IngestionJob job =
                IngestionJob.accepted(IngestionJob.Type.HISTORICAL, "9165")
