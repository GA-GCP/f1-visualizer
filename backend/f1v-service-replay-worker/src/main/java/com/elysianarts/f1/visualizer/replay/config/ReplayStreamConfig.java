...
public class ReplayStreamConfig {
    ...
    public StreamMessageListenerContainer<String, MapRecord<String, String, String>>
        ...
        Subscription subscription =
                container.receiveAutoAck(
                        ...
                        Consumer.from(
                                ReplayCommandStream.CONSUMER_GROUP, "worker-" + UUID.randomUUID()),
                        StreamOffset.create(ReplayCommandStream.KEY, ReadOffset.lastConsumed()),
                        listener);

        container.start();
        log.info(
                "replay command listener started stream={} group={} active={}",
                ReplayCommandStream.KEY,
    ...
    /**
     * Creates the group, and the stream with it. Reading from a group that does not exist is an
     * error, and on a fresh environment nothing has published yet.
     */
