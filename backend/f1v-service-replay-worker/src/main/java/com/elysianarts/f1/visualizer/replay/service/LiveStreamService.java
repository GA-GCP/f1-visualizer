 ...
 *
 * <p><b>Reconnect contract (R2).</b> The connection is opened with
 * {@code cleanSession=true}, which means the broker discards subscriptions on
 * every disconnect. Paho's {@code automaticReconnect} restores the <em>session</em>
 * but not the subscriptions, so {@link MqttCallbackExtended#connectComplete}
 * re-subscribes on every successful connect — the original code implemented the
 * plain {@code MqttCallback}, so after the first dropped connection the feed went
 * silent with no error at all.</p>
 *
 * <p>Paho also retries with the connect options it was originally given, so once
 * the OpenF1 token expires every automatic retry fails with the stale password.
 * {@code connectionLost} therefore refreshes the token and, if it actually
 * changed, rebuilds the client with fresh options rather than leaving Paho to
 * retry a credential that can no longer succeed.</p>
 */
@Slf4j
@Service
public class LiveStreamService {
    ...
    public LiveStreamService(RedisTemplate<String, Object> redisTemplate,
        ...
        this.reconnectCounter = Counter.builder("f1v.mqtt.reconnects")
    ...
    /**
     * Re-establishes the subscriptions. Called on the initial connect and again
     * from {@code connectComplete} after every reconnect, because a clean session
    ...
    /**
     * Rebuilds the connection with a token fetched after the drop. Only worth
     * doing when the token actually moved — otherwise Paho's own retry loop is
     * already doing the right thing and tearing it down would be a regression.
     */
    void reconnectWithFreshToken() {
        ...
        log.info("mqtt token changed after refresh — rebuilding connection session_key={}", currentSessionKey);
        try {
            connect(currentSessionKey);
        } catch (RuntimeException e) {
            log.error("mqtt rebuild with refreshed token failed session_key={}", currentSessionKey, e);
        }
    }
    ...
    private final class SessionCallback implements MqttCallbackExtended {
        ...
        public void connectComplete(boolean reconnect, String serverURI) {
            ...
            try {
                ...
                log.error("mqtt re-subscribe failed after connect session_key={}", currentSessionKey, e);
            }
        }

        @Override
        public void connectionLost(Throwable cause) {
            log.warn("mqtt connection lost session_key={} — refreshing token", currentSessionKey, cause);
