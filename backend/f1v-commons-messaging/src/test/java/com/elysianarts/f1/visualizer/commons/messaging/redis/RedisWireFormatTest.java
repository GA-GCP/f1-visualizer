 ...
 *
 * <p>These assertions used to live two modules away from the configuration they protect, against a
 * hand-copied mirror of it that was free to drift. They now run against {@link
 * RedisMessagingConfig#redisObjectMapper()} itself, beside the single definition of that mapper
 * (C5, C1). The interface has broken twice (#36, #43).
 */
class RedisWireFormatTest {
    ...
    void locationData_MatchesFrontendLocationPacket() {
        ...
        for (String key :
                new String[] {
                    "session_key", "meeting_key", "date", "driver_number", "x", "y", "z"
                }) {
            assertTrue(node.has(key), "Expected '" + key + "'");
        }
        assertFalse(node.has("sessionKey"), "camelCase 'sessionKey' must not appear");
    ...
    void carData_StillReadsOpenF1InboundNaming() {
        OpenF1CarData data =
                redisMapper.readValue(
                        "{\"session_key\":9165,\"driver_number\":1,\"speed\":310,\"n_gear\":8}",
                        OpenF1CarData.class);
    ...
    void carData_RoundTrips() {
        ...
        OpenF1CarData deserialized =
                redisMapper.readValue(
                        redisMapper.writeValueAsString(original), OpenF1CarData.class);
