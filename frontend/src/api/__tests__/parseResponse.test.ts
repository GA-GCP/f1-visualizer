...
describe('parseResponse', () => {
    ...
    it('returns the parsed value when the shape matches', () => {
        expect(parseResponse(schema, { id: 1, code: 'VER' }, 'test')).toEqual({
            id: 1,
            code: 'VER',
        });
    });
    ...
    it('throws a distinguishable error when the contract has drifted', () => {
        ...
        expect(() => parseResponse(schema, { id: 1, driverCode: 'VER' }, 'GET /drivers')).toThrow(
            SchemaMismatchError,
        );
    });
    ...
    it('rejects a wrong primitive type, not just a missing key', () => {
        ...
        expect(() => parseResponse(schema, { id: '1', code: 'VER' }, 'test')).toThrow(
            SchemaMismatchError,
        );
    });
});
...
describe('wire schemas', () => {
    ...
    it('rejects a driver profile missing its stats block', () => {
        expect(
            z.safeParse(driverProfileSchema, {
                id: 1,
                code: 'VER',
                name: 'Max Verstappen',
                team: 'Red Bull',
