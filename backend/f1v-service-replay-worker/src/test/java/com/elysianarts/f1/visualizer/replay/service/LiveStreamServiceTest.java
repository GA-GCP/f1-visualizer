...
class LiveStreamServiceTest {
    ...
    @Mock private OpenF1AuthService authService;
    ...
    private LiveStreamService newService() {
        ...
        LiveStreamService service =
                new LiveStreamService(redisTemplate, jsonMapper, authService, meterRegistry);
    ...
    private static MqttCallbackExtended callbackOf(MqttClient client) {
        ArgumentCaptor<MqttCallbackExtended> captor =
    ...
    void connect_SetsUpCallback_AndParsesTelemetryCorrectly() throws Exception {
        ...
        try (MockedConstruction<MqttClient> mockedClient = mockConstruction(MqttClient.class)) {
            ...
            verify(redisTemplate, times(1))
                    .convertAndSend(eq(RedisTopics.TELEMETRY), carDataCaptor.capture());
    ...
    /**
     * R2: a clean session drops its subscriptions on every disconnect, so a reconnect that does not
     * re-subscribe leaves the feed silent with no error.
     */
    @Test
    ...
    /**
     * R2: Paho retries with the options it was originally handed, so an expired token can only be
     * recovered by rebuilding the client.
     *
     * <p>The rebuild is driven directly rather than through {@code connectionLost} because
     * Mockito's {@code mockConstruction} is scoped to the calling thread, and the production path
     * deliberately hands the blocking {@code connect()} to a separate executor. {@link
     * #connectionLost_RefreshesTheToken} covers the hand-off.
     */
    @Test
    ...
    /**
     * A transient blip with an unchanged token is Paho's automatic reconnect to handle; tearing the
     * client down would be a regression.
     */
    @Test
    ...
    void connect_ParsesLocationData_WhenTopicIsLocation() throws Exception {
        ...
        try (MockedConstruction<MqttClient> mockedClient = mockConstruction(MqttClient.class)) {
            ...
            String locationPayload =
                    "{\"driver_number\":44, \"x\": 1456, \"y\": -2340, \"z\": 150}";
            ...
            verify(redisTemplate, times(1))
                    .convertAndSend(eq(RedisTopics.LOCATION), locCaptor.capture());
