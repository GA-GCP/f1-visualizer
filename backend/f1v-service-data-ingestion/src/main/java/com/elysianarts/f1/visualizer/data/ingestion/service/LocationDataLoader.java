...
public class LocationDataLoader {
    ...
    public void loadLocationsIntoBigQuery(long sessionKey) {
        ...
        log.info(
                "location load window session_key={} from={} to={}",
                sessionKey,
                windowStart,
                raceEnd);
        ...
        while (windowStart.isBefore(raceEnd)) {
            ...
            try {
                List<OpenF1LocationData> data =
                        openF1Client.getLocationData(sessionKey, windowStart, windowEnd);
                ...
                log.error(
                        "location window failed session_key={} from={} to={}",
                        sessionKey,
                        windowStart,
                        windowEnd,
                        e);
        ...
        log.info(
                "location load complete session_key={} rows={} table={}.{}",
                sessionKey,
    ...
    /**
     * See {@link HistoricalDataLoader} — replace rather than duplicate, with the date range the
     * partitioned table requires (R4, P2).
     */
    private void deleteExistingRows(long sessionKey, OffsetDateTime from, OffsetDateTime to) {
        ...
        try {
            ...
            throw new IllegalStateException(
                    "Could not clear existing locations for session " + sessionKey, e);
        }
    }
