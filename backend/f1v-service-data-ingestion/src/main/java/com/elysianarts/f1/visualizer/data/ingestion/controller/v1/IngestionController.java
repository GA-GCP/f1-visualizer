 ...
 * The ingestion API.
 *
 * <p><b>R1: stateless.</b> This controller used to call the replay engine's methods directly, which
 * only worked while the engine was a singleton in the same JVM — and Cloud Run runs several.
 * Playback commands go onto a Redis stream for the replay worker to pick up, and status is read
 * from Redis, so any instance can serve any request.
 */
@Slf4j
@RestController
...
public class IngestionController {
    ...
    @PostMapping("/command")
    public ResponseEntity<IngestionAck> issueIngestionCommand(
            @Valid @RequestBody IngestionCommandRequest request) {
        log.info(
                "ingestion command mode={} session_key={}",
                request.getMode(),
                request.getSessionKey());

        ReplayCommand command =
                request.getMode() == IngestionMode.SIMULATION
        ...
        return ResponseEntity.ok(
                new IngestionAck(what + " initiated for session " + request.getSessionKey()));
    }

    /**
     * R3: 202 with a job id, not a 504 with the work still running. Four loaders plus the stats
     * recompute is minutes of work, well past the gateway's deadline.
     */
                ...
                .orElseThrow(
                        () ->
                                new ResponseStatusException(
                                        HttpStatus.NOT_FOUND, "No such ingestion job"));
    }
    ...
    public ResponseEntity<IngestionAck> seekSimulation(
    ...
    /**
     * R1: what the worker is doing, read from Redis. There was previously no way to ask — the
     * answer lived in one instance's memory, and the instance that received the question was
     * usually not that one.
     */
