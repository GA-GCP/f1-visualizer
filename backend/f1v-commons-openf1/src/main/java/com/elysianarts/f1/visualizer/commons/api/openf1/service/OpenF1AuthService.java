 ...
 *
 * <p><b>Threading (R7).</b> The token is read from HTTP, scheduler and MQTT
 * threads, so it lives in an {@link AtomicReference} rather than a plain field.
 * The refresh itself no longer runs on Boot's single shared {@code TaskScheduler}
 * thread on a fixed rate, where a slow {@code /token} response froze the 250 ms
 * replay tick for its duration, and no longer runs in {@code @PostConstruct},
 * where it delayed readiness on every deploy. The first refresh is scheduled
 * from {@link ApplicationReadyEvent} and each subsequent one is scheduled from
 * the {@code expires_in} the server actually returned.</p>
 */
@Slf4j
@Service
public class OpenF1AuthService {
    ...
    public OpenF1AuthService(RestClient.Builder restClientBuilder,
    ...
    public OpenF1AuthService(RestClient restClient,
                             OpenF1Credentials credentials,
                             TaskScheduler taskScheduler) {
    ...
    /**
     * Off the main thread, so a slow or unreachable OpenF1 does not hold up the
     * container's readiness on every deploy.
     */
    ...
    /**
     * Fetches a new token. Returns how long to wait before the next refresh —
     * derived from the server's {@code expires_in} on success, and a fixed
    ...
    public Duration refreshToken() {
        if (credentials == null || credentials.username() == null || credentials.password() == null) {
            log.warn("openf1 credentials missing — skipping authentication (expected in tests and local runs)");
    ...
    /**
     * The current token, or a thrown exception. Callers that would otherwise send
     * {@code Authorization: Bearer null} and read the resulting 401 as "no data"
     * use this instead (S6, R5).
     */
    public String requireAccessToken() {
        String token = currentAccessToken.get();
        if (token == null) {
            throw new IllegalStateException("No OpenF1 access token available — authentication has not succeeded");
        }
        return token;
    }
