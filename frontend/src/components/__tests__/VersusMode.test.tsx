...
const renderAt = (route: string) => renderWithProviders(<VersusMode />, { route });
...
describe('VersusMode Page', () => {
    ...
    it('shows loading state initially, then renders comparison engine when data arrives', async () => {
        ...
        // Wait on the radar, not on 'COMPARISON ENGINE': the ghost loader
        // renders that heading too, so waiting for it was satisfied by the
        // loader itself and the assertion below only passed by timing luck.
        await waitFor(() => {
