...
public class RaceAnalysisService {
    ...
    public List<LapDataRecord> getSessionLapTimes(long sessionKey) {
        ...
        try {
            TableResult result = queryRunner.query(
            ...
            for (FieldValueList row : result.iterateAll()) {
                records.add(LapDataRecord.builder()
                        ...
                        .dateStart(row.get("date_start").isNull() ? null : row.get("date_start").getStringValue())
                        .isPitOutLap(row.get("is_pit_out_lap").isNull() ? null : row.get("is_pit_out_lap").getBooleanValue())
                        .build());
            }
     ...
     *
     * <p>This used to build a ten-CTE query per request, including
     * {@code COUNTIF(throttle > 95)} across the whole telemetry table with no
     * partition filter — seconds of latency and bytes billed that grew with every
     * session ever ingested, for numbers that only change when a session is
     * loaded. The head-to-head page issues two of these per comparison. The
     * ingestion run now computes them; this reads roughly twenty rows.</p>
     */
    ...
    public DriverProfile.DriverStats getDriverStats(int driverNumber) {
        ...
        try {
            TableResult result = queryRunner.query(QueryJobConfiguration.newBuilder(query)
                    .addNamedParameter("driverNumber", QueryParameterValue.int64(driverNumber)));
            ...
            log.info("driver stats missing driver_number={} — no ingestion run has covered this driver", driverNumber);
    ...
    static DriverProfile.DriverStats toStats(FieldValueList row) {
        ...
        int speed = row.get("avg_position").isNull() ? 50 : clamp(
                Math.round(100 - (row.get("avg_position").getDoubleValue() - 1) * (90.0 / 19)));

        // Consistency: STDDEV of finishing positions.
        // stddev 1 -> 93, 3 -> 79, 5 -> 65, 10 -> 30
        int consistency = row.get("position_stddev").isNull() ? 50 : clamp(
                Math.round(100 - row.get("position_stddev").getDoubleValue() * 7));

        // Aggression: full-throttle percentage in telemetry.
        int aggression = row.get("full_throttle_pct").isNull() ? 50 : clamp(
                Math.round(row.get("full_throttle_pct").getDoubleValue() * 1.5));

        // Tire management: average stint length.
        // 10 laps -> 32, 15 -> 48, 20 -> 64, 25 -> 80, 30+ -> 96+
        int tireMgmt = row.get("avg_stint_length").isNull() ? 50 : clamp(
                Math.round((row.get("avg_stint_length").getDoubleValue() / 25.0) * 80));

        int totalRaces = row.get("total_races").isNull() ? 0 : (int) row.get("total_races").getLongValue();
        // Experience: total race entries, ceiling at 80 races.
        int experience = row.get("total_races").isNull()
                ? 30
                : (int) Math.min(99, Math.round((totalRaces / 80.0) * 99));

        List<String> teams = row.get("teams_list").isNull()
                ? List.of()
        ...
        return DriverProfile.DriverStats.builder()
                ...
                .totalPoints(row.get("total_points").isNull() ? 0 : (int) row.get("total_points").getLongValue())
                .bestChampionshipFinish(row.get("best_finish").isNull() ? 0 : (int) row.get("best_finish").getLongValue())
    ...
    static final DriverProfile.DriverStats DEFAULT_STATS = DriverProfile.DriverStats.builder()
