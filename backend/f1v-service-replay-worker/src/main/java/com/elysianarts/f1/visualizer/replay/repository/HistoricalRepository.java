...
public class HistoricalRepository {
     ...
     *
     * <p>Two cheap queries rather than one expensive one (P2). The telemetry table
     * now requires a partition filter, and this query had none — it was a MIN/MAX
     * across every partition. The session's advertised window comes from the small
     * unpartitioned {@code sessions} table first, and bounds the exact MIN/MAX that
     * follows.</p>
     */
    public SessionBounds fetchTelemetryBounds(long sessionKey) {
        OffsetDateTime[] window = fetchSessionWindow(sessionKey);
        if (window == null) {
            log.error("replay bounds unavailable session_key={} reason=session_row_missing_dates "
                    + "— run POST /api/v1/ingestion/load-reference for the session's year", sessionKey);
            return null;
        }
        ...
        try {
            ...
            throw new BigQueryAccessException("Failed to read replay bounds for session " + sessionKey, e);
        }
    }
    ...
    private OffsetDateTime[] fetchSessionWindow(long sessionKey) {
        ...
        try {
            TableResult result = queryRunner.query(query);
            for (FieldValueList row : result.iterateAll()) {
                OffsetDateTime start = microsToOffsetDateTime(row.get("date_start").getTimestampValue());
                OffsetDateTime end = row.get("date_end").isNull()
                        ? start.plusHours(4)
                        : microsToOffsetDateTime(row.get("date_end").getTimestampValue());
                return new OffsetDateTime[]{start, end};
            }
        } catch (Exception e) {
            throw new BigQueryAccessException("Failed to read the session window for session " + sessionKey, e);
        }
        return null;
    }
    ...
    public List<OpenF1CarData> fetchTelemetryWindow(long sessionKey, OffsetDateTime from, OffsetDateTime to) {
        ...
        try {
            ...
            throw new BigQueryAccessException("Failed to read the telemetry window for session " + sessionKey, e);
        }
    }
