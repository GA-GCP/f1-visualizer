...
public class F1VUserController {
    ...
    public ResponseEntity<?> getCurrentUser(@AuthenticationPrincipal Jwt jwt) {
        ...
        if (email == null || email.isBlank()) {
            log.warn(
                    "JWT for subject '{}' is missing the 'email' claim. "
                            + "Ensure the Auth0 Post-Login Action includes email in the access token.",
                    authSubId);
            return ResponseEntity.badRequest()
                    .body(
                            Map.of(
                                    "error",
                                    "JWT missing 'email' claim. Configure Auth0 Post-Login Action to include email in access token.",
                                    "status",
                                    400));
        }
