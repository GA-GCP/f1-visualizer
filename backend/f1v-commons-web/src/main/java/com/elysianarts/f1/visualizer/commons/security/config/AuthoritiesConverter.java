 ...
 *
 * <p>Spring's default converter reads the {@code scope}/{@code scp} claim and
 * prefixes each value with {@code SCOPE_}. With Auth0 RBAC enabled the API's
 * permissions arrive in a separate top-level {@code permissions} array, already
 * namespaced as {@code ingest:admin}, and nothing was reading it — which is why
 * the codebase contained no authorization rule beyond "has a token" (S1).</p>
 *
 * <p>Both sources are unioned so scope-based rules keep working alongside the new
 * permission-based ones.</p>
 */
